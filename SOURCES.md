...
- **⚠️ Home-price universe (D35, 2026-08-06):** the borough/citywide DOF median used by the affordability engine is now restricted to **residential ownership sales** — `building_class_category` prefixes **01/02/03** (1–3 family), **04** (TC1 condos), **09/10** (co-ops), **12/13/15/17** (condos) — via `starts_with(building_class_category,'NN ')` on DOF Rolling (`usep-8jbt`), latest complete month. This EXCLUDES commercial, hotels (incl. $170M condo-hotels), offices, stores, condo parking/storage, vacant land, and rental apartment BUILDINGS (investment multifamily). Previously the median filtered only `sale_price>10000`, mixing all property types → inflated "median home price." Arm's-length can't be perfectly isolated in DOF (the $10k floor removes $0/nominal). Corrected citywide median: **$880k** (was $940k). The **Transactions feed itself still shows ALL recorded sales** (labeled as such) — only the affordability home-price basis is residential-restricted.
