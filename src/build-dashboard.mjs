...
function renderBrief(){
  ...
  h+=section('The Brief · what matters now',
    '<div class="headline"><p class="brief-lead">'+esc(B.headline)+'</p><p class="brief-sum">'+esc(B.summary)+'</p></div>');

  h+=section('Key signals',
    '<div class="grid">'+(B.insights||[]).map(i=>
      '<div class="brief-card"><div class="bt">'+esc(i.title)+'</div><div class="bd">'+esc(i.detail)+'</div><span class="feedchip">'+esc(i.feed)+'</span></div>').join('')+'</div>');

  if((B.neighborhoods||[]).length) h+=section('Neighborhood spotlights',
    '<div class="grid">'+B.neighborhoods.map(n=>
      '<div class="brief-card"><div class="bt">'+esc(n.area)+'</div><div class="bd">'+esc(n.narrative)+'</div></div>').join('')+'</div>');
